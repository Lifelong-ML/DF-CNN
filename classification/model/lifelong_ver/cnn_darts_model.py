...
from utils.utils import new_weight, new_bias, new_ELLA_KB_param, get_list_of_valid_tensors
...
class LL_HPS_CNN_DARTS_net(Lifelong_Model_Frame):
    ...
    def define_eval(self):
        with tf.name_scope('Model_Eval'):
            ...
            if self.task_is_new:
                self.eval_for_new_task = [tf.nn.softmax(task_model) for task_model in self.darts_eval_models]
                self.pred_for_new_task = [tf.argmax(task_model, 1) for task_model in self.darts_eval_models]
    ...
    def define_accuracy(self):
        with tf.name_scope('Model_Accuracy'):
            ...
            if self.task_is_new:
                self.accuracy_for_new_task = [tf.reduce_sum(tf.cast(tf.equal(tf.argmax(task_model, 1), tf.cast(self.y_batch[self.current_task], tf.int64)), tf.float32)) for task_model in self.darts_eval_models]
