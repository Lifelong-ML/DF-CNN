...
def train_lifelong_LASEM(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, classification_prob, useGPU=False, GPU_device=0, save_param=False, param_folder_path='saved_param', save_param_interval=100, save_graph=False, tfInitParam=None, run_cnt=0):
    ...
    assert (model_architecture == 'hybrid_dfcnn_auto_sharing_em' or model_architecture == 'hybrid_hps_cnn_em' or model_architecture == 'hybrid_tf_cnn_em'), "Use train function appropriate to the architecture"
        ...
        with tf.Session(config=config) as sess:
            ...
            while learning_step < min(learning_step_max, epoch_bias + patience):
                ...
                if learning_step > 0:
                    ...
                    for batch_cnt in range(num_train[task_for_train]//batch_size):
                        ...
                        if learning_model.task_is_new:
                            config_posterior = sess.run(learning_model.posterior, feed_dict={learning_model.model_input[task_model_index]: batch_train_x, learning_model.true_output[task_model_index]: batch_train_y, learning_model.dropout_prob: 1.0})
                ...
                for tmp_cnt, (task_index_to_eval) in enumerate(task_training_order[:train_task_cnt+1]):
                    ...
                    if learning_model.task_is_new and (learning_model.current_task == task_model_index_to_eval):
                        model_error = learning_model.accuracy_for_train[best_config]
                    else:
                        model_error = learning_model.accuracy[task_model_index_to_eval]

                    for batch_cnt in range(num_train[task_index_to_eval]//batch_size):
                        train_error_tmp[task_index_to_eval] = train_error_tmp[task_index_to_eval] + sess.run(model_error, feed_dict={learning_model.model_input[task_model_index_to_eval]: train_data[task_index_to_eval][0][batch_cnt*batch_size:(batch_cnt+1)*batch_size, :], learning_model.true_output[task_model_index_to_eval]: train_data[task_index_to_eval][1][batch_cnt*batch_size:(batch_cnt+1)*batch_size], learning_model.dropout_prob: 1.0})
                    ...
                    for batch_cnt in range(num_test[task_index_to_eval]//batch_size):
                        test_error_tmp[task_index_to_eval] = test_error_tmp[task_index_to_eval] + sess.run(model_error, feed_dict={learning_model.model_input[task_model_index_to_eval]: test_data[task_index_to_eval][0][batch_cnt*batch_size:(batch_cnt+1)*batch_size, :], learning_model.true_output[task_model_index_to_eval]: test_data[task_index_to_eval][1][batch_cnt*batch_size:(batch_cnt+1)*batch_size], learning_model.dropout_prob: 1.0})
...
def train_lifelong_LASEM_fixedProb(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, classification_prob, useGPU=False, GPU_device=0, save_param=False, param_folder_path='saved_param', save_param_interval=100, save_graph=False, tfInitParam=None, run_cnt=0):
    ...
    assert (model_architecture == 'hybrid_dfcnn_auto_sharing_em_fixed'), "Use train function appropriate to the architecture"
...
def train_lifelong_DARTS_hybridNN(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, classification_prob, useGPU=False, GPU_device=0, save_param=False, param_folder_path='saved_param', save_param_interval=100, save_graph=False, tfInitParam=None, run_cnt=0):
    ...
    assert (model_architecture == 'hybrid_hps_cnn_darts' or model_architecture == 'hybrid_dfcnn_darts'), "Use train function appropriate to the architecture"
        ...
        with tf.Session(config=config) as sess:
            ...
            while learning_step < min(learning_step_max, epoch_bias + patience):
                ...
                if learning_step > 0:
                    ...
                    for batch_cnt in range(num_train[task_for_train]//batch_size):
                        ...
                        if learning_model.task_is_new:
                            ...
                            sess.run(learning_model.selection_update, feed_dict={learning_model.model_input[task_model_index]: batch_train_x, learning_model.true_output[task_model_index]: batch_train_y, learning_model.epoch: learning_step-1, learning_model.dropout_prob: 0.5})
                ...
                for tmp_cnt, (task_index_to_eval) in enumerate(task_training_order[:train_task_cnt+1]):
                    ...
                    task_model_index_to_eval = learning_model.find_task_model(task_index_to_eval)
                    if learning_model.task_is_new and (learning_model.current_task == task_model_index_to_eval):
                        model_error = learning_model.accuracy_for_new_task[best_config]
                    else:
                        model_error = learning_model.accuracy[task_model_index_to_eval]
