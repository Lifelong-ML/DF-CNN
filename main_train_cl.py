...
from classification.gen_data import fashion_mnist_data, stl10_data, print_experiment_design ## New format of experiment design
...
def main():
    ...
    parser.add_argument('--em_cnt_prior', help='EM method - prior probability is NOT based on the count of mini-batch', action='store_false', default=True)
    ...
    train_hyperpara['data_augment'] = False
    ...
    if 'mnist' in data_type and 'fashion' not in data_type:
        ...
        train_hyperpara['num_run_per_model'] = 5
        train_hyperpara['train_valid_data_group'] = list(range(5))
        ...
        #data_hyperpara['task_type'] = data_type[data_type.find('_')+1:]
        ...
        data_hyperpara['data_percent'] = args.data_percent
        data_hyperpara['validation_data_ratio'] = 15
        data_hyperpara['num_classes'] = 2
        data_hyperpara['num_tasks'] = 20

        data_file_name = 'fashionmnist_p'+str(data_hyperpara['data_percent'])+'_v'+str(data_hyperpara['validation_data_ratio'])+'_t'+str(data_hyperpara['num_tasks'])

        categorized_train_data, categorized_test_data, experiments_design = fashion_mnist_data(experiment_file_base_name=data_file_name, valid_data_ratio_to_whole=float(data_hyperpara['validation_data_ratio'])/100.0, num_train_group=data_hyperpara['num_train_group'], num_tasks=data_hyperpara['num_tasks'], data_percent=float(data_hyperpara['data_percent'])/100.0, num_classes_per_task=data_hyperpara['num_classes'])
        print_experiment_design(experiments_design, data_type, print_info=True)
        classification_prob = True
    ...
    ## Model Set-up
    model_architecture, model_hyperpara = model_setup(data_type, data_hyperpara['image_dimension'], args.model_type, args.test_type, args.cnn_padtype_valid, args.skip_connect_test_type, args.highway_connect_test_type, args.num_clayers, args.darts_approx_order)
    ...
    if 'fashion' in data_type or 'stl10' in data_type:
        saved_result = train_run_for_each_model_v2(model_architecture, model_hyperpara, train_hyperpara, [categorized_train_data, categorized_test_data, experiments_design], data_type, mat_file_name, classification_prob, saved_result=None, useGPU=use_gpu, GPU_device=gpu_device_num, doLifelong=do_lifelong, saveParam=args.saveparam, saveParamDir=save_param_path, saveGraph=args.savegraph, tfInitParamPath=tensorfactor_param_path)
    else:
        saved_result = train_run_for_each_model(model_architecture, model_hyperpara, train_hyperpara, [train_data, validation_data, test_data], data_type, mat_file_name, classification_prob, saved_result=None, useGPU=use_gpu, GPU_device=gpu_device_num, doLifelong=do_lifelong, saveParam=args.saveparam, saveParamDir=save_param_path, saveGraph=args.savegraph, tfInitParamPath=tensorfactor_param_path)
